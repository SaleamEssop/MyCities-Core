...
- [x] **Laravel 8 base** (from MyCities-Cline backend) + Inertia + Vue 3 (Mix for assets)
- [x] Preserve PD + Calculator after merge
- [x] Inertia route: `/inertia-welcome` → `Welcome.vue` (test page)
- [ ] Add Quasar as Vue plugin (optional; retain look/feel)
- [ ] **Run:** `composer install` and `npm install && npm run dev` (in Docker or local)
