...
<title>
@if(isset($calculator_mode) && $calculator_mode === 'calculator-php')
CalculatorPHP ({{ $calculator_revision ?? 'Rev1' }}) - Clean Implementation
@else
MyCities Billing – UI Rev 2.1 (Optimized)
@endif
</title>
...
@if(isset($calculator_mode) && $calculator_mode === 'calculator-php')
CalculatorPHP ({{ $calculator_revision ?? 'Rev1' }}) - Clean
@else
Billing Calculator PHP
@endif
</div>
    </div>
  ...
  <div class="main-content">
    ...
    <div id="period-mode-container">
      <!-- Billing Mode and Template - One Row, Two Columns (Matching HTML guide) -->
      <div style="margin-bottom:24px;">
        <div style="display:flex; gap:30px;">
          <!-- Billing Mode Field -->
          <div style="flex:1;">
            <label style="display:block; margin-bottom:5px; font-weight:600;">Billing Mode</label>
            <select id="billing_mode_select_period" class="input-select" onchange="window.switchBillingModeFromDropdown(this.value)" style="width:100%; border:1px solid #4da3d9; border-radius:6px; padding:8px 12px; font-size:16px;">
          ...
          <div style="flex:1;">
            ...
            <select id="tariff_template_select" class="input-select" onchange="window.loadTariffTemplate(this.value, 'period')" style="width:100%; border:1px solid #4da3d9; border-radius:6px; padding:8px 12px; font-size:16px;">
      ...
      <div class="section">
        ...
        <div class="section-content">
          <!-- Billing Period Row - One Row, Four Columns (Matching HTML guide) -->
          <div style="display:flex; align-items:flex-end; gap:40px;">
            ...
            <div style="flex:1;">
              ...
              <input type="number" id="bill_day" class="input-number" min="1" max="31" value="20" style="width:100%; border:1px solid #4da3d9; border-radius:6px; padding:8px 12px; font-size:16px;">
            </div>
            
            <!-- Start Month Field -->
            <div style="flex:1;">
              <label style="display:block; margin-bottom:5px; font-weight:600;">Start Month</label>
              <input type="month" id="start_month" class="input-date" value="2026-01" style="width:100%; border:1px solid #4da3d9; border-radius:6px; padding:8px 12px; font-size:16px;">
            </div>
            
            <!-- Current Date Field -->
            <div style="flex:1;">
              <label style="display:block; margin-bottom:5px; font-weight:600;">Current date</label>
              <input type="date" id="current_date" class="input-date" style="width:100%; border:1px solid #4da3d9; border-radius:6px; padding:8px 12px; font-size:16px;">
            </div>
            
            <!-- Apply Button -->
            <div>
              <button onclick="window.add_period()" style="background:#8fc7d8; border:none; padding:6px 12px; border-radius:6px; font-size:16px; cursor:pointer; white-space:nowrap;">Apply</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="section">
        ...
        <div class="section-content">
          <!-- Period Cards (Mobile-style design) -->
          <table id="period_table" style="width:100%; margin-bottom:20px; border-collapse:collapse; border-spacing:0;">
          ...
          <table id="period_reading_table" style="width:100%; border-collapse:separate; border-spacing:0;">
            <thead>
              <tr>
                <th style="padding:12px 16px; text-align:left; color:#000000; font-size:16px; font-weight:400;">Date</th>
                <th style="padding:12px 16px; text-align:left; color:#000000; font-size:16px; font-weight:400;">Reading</th>
                <th style="padding:12px 16px; text-align:left; color:#000000; font-size:16px; font-weight:400;">Cost (R)</th>
                <th style="padding:12px 16px;"></th>
              </tr>
            </thead>
            <tbody></tbody>
          </table>
          <!-- Action Buttons (Centered - Matching HTML guide) -->
          <div class="action-row" style="margin-top:20px; margin-bottom:20px; text-align:center;">
            <!-- Add Reading Button -->
            <button onclick="window.add_reading()" 
            ...
            <!-- Calculate Button -->
            <button id="calculate_btn" onclick="calculate()" 
          ...
          <div id="calculate_error" style="color:var(--red); display:none; font-size:14px; text-align:right; margin-top:8px;"></div>

          <div id="period_dashboard" style="margin-top:30px;">
            <!-- Summary Metrics (Light Blue Bar - Matching HTML guide) -->
            <div style="background:#9DCADA; display:flex; justify-content:space-around; padding:25px; border-radius:8px;">
              <div style="text-align:center;">
                <div style="font-size:14px; color:#000000; font-weight:600;">Daily Usage</div>
                <div style="font-size:28px; font-weight:700; color:#000000; margin-top:6px;" id="period_dashboard_daily_usage">—</div>
              </div>
              <div style="text-align:center;">
                <div style="font-size:14px; color:#000000; font-weight:600;">Daily Cost</div>
                <div style="font-size:28px; font-weight:700; color:#000000; margin-top:6px;" id="period_dashboard_daily_cost">—</div>
              </div>
              <div style="text-align:center;">
                <div style="font-size:14px; color:#000000; font-weight:600;">
                  <span id="period_dashboard_total_cost_label">Projected Total</span>
                </div>
                <div style="font-size:28px; font-weight:700; color:#000000; margin-top:6px;" id="period_dashboard_total_cost">R 0.00</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <div id="sector-mode-container" style="display:none;">
      ...
      <div style="margin-bottom:24px;">
        <div style="display:flex; gap:30px;">
          ...
          <div style="flex:1;">
            <label style="display:block; margin-bottom:5px; font-weight:600;">Billing Mode</label>
            <select id="billing_mode_select" class="input-select" onchange="window.switchBillingModeFromDropdown(this.value)" style="width:100%; border:1px solid #4da3d9; border-radius:6px; padding:8px 12px; font-size:16px;">
          ...
          <div style="flex:1;">
            <label style="display:block; margin-bottom:5px; font-weight:600;">Template</label>
            <select id="sector_tariff_template_select" class="input-select" onchange="window.loadTariffTemplate(this.value, 'sector')" style="width:100%; border:1px solid #4da3d9; border-radius:6px; padding:8px 12px; font-size:16px;">
              <option value="">-- Select Template --</option>
            </select>
          </div>
        </div>
        ...
        <div class="compact-field">
          ...
          <div style="position:relative;">
            <input type="date" id="sector_date_picker" class="compact-input" style="padding-right:40px;" min="">
      ...
      <!-- REMOVED: 3️⃣ SECTORS & READINGS (Date-to-Date mode only - Readings input section) -->
      <!-- SECTORS ARE DERIVED DATA - Users must never input or modify sectors directly -->
      <!-- All readings must be entered at the Period level only -->
      <!-- Sector data is displayed in the "5️⃣ Sector Breakdown (Derived Data)" section below -->

      <!-- 5️⃣ SECTOR BREAKDOWN (Derived Data) -->
      <div class="section">
        <div class="section-header">5️⃣ Sector Breakdown (Derived Data)</div>
        <div class="section-content">
          <div style="font-size:14px; color:var(--muted); margin-bottom:16px; padding:12px; background:rgba(59, 130, 246, 0.1); border-left:3px solid #3b82f6; border-radius:4px;">
            <strong>Note:</strong> Sectors are derived automatically from meter readings and period boundaries and cannot be edited directly. All readings must be entered at the Period level.
          </div>
...
<!-- Force PHP Calculator Mode (Isolated) -->
<script>
// Force PHP calculator mode for this isolated view
@if(isset($calculator_mode) && $calculator_mode === 'calculator-php')
// CalculatorPHP mode - NEW CLEAN IMPLEMENTATION
window.FORCE_PHP_CALCULATOR = true;
window.CALCULATOR_MODE = 'calculator-php';
window.CALCULATOR_REVISION = '{{ $calculator_revision ?? 'Rev1' }}';
@else
// Legacy PHP calculator mode
window.FORCE_PHP_CALCULATOR = true;
window.CALCULATOR_MODE = 'php';
@endif
</script>

<!-- Load CalculatorPHP UI (NEW ISOLATED) or Legacy PHP UI -->
@if(isset($calculator_mode) && $calculator_mode === 'calculator-php')
<!-- CalculatorPHP UI - NEW ISOLATED IMPLEMENTATION -->
<script src="{{ url('/js/calculator-php-ui.js') }}?v={{ time() }}"></script>
@else
<!-- Legacy PHP-only UI management JavaScript -->
<script src="{{ url('/js/billing-calculator-php-ui-only.js') }}?v={{ time() }}"></script>
@endif

<!-- Date Constraint Handlers (must be after PHP UI script loads) -->
<script>
// Update sector_date_picker constraint when date changes (DATE TO DATE: forward only)
document.addEventListener('DOMContentLoaded', function() {
    const sectorDatePicker = document.getElementById('sector_date_picker');
    if (sectorDatePicker) {
        // Update constraint whenever a date is selected
        sectorDatePicker.addEventListener('change', function() {
            // After selecting a date, update min to be the day after selected date (cannot go backwards)
            if (this.value) {
                const selectedDate = new Date(this.value);
                selectedDate.setDate(selectedDate.getDate() + 1); // Day after selected date
                this.min = selectedDate.toISOString().slice(0, 10);
            }
            
            // Also update the global constraint function
            if (typeof updateSectorDatePickerConstraint === 'function') {
                updateSectorDatePickerConstraint();
            }
        });
        
        // Initialize constraint on page load
        setTimeout(function() {
            if (typeof updateSectorDatePickerConstraint === 'function') {
                updateSectorDatePickerConstraint();
            }
...
const LaravelAPI = {
    ...
    getTariffTemplates: async function() {
        console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 1 - Function called');
        console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 2 - apiBaseUrl:', apiBaseUrl);
        console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 3 - Full URL:', `${apiBaseUrl}/tariff-templates`);
        
        try {
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 4 - Making fetch request...');
            const response = await fetch(`${apiBaseUrl}/tariff-templates`);
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 5 - Response received:', response);
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 6 - Response status:', response.status);
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 7 - Response ok:', response.ok);
            
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 8 - Parsing JSON...');
            const data = await response.json();
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 9 - Data received:', data);
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 10 - Data success:', data.success);
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 11 - Data.data:', data.data);
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 12 - Data.data type:', Array.isArray(data.data));
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 13 - Data.data length:', data.data?.length);
            
            const result = data.success ? data.data : [];
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 14 - Returning result:', result);
            console.log('[TEMPLATE DEBUG] getTariffTemplates: Step 15 - Result length:', result.length);
            return result;
        } catch (error) {
            console.error('[TEMPLATE DEBUG] getTariffTemplates: ERROR -', error);
            console.error('[TEMPLATE DEBUG] getTariffTemplates: Error message:', error.message);
            console.error('[TEMPLATE DEBUG] getTariffTemplates: Error stack:', error.stack);
            return [];
        }
    },
...
// Note: currentTariffTemplate, currentTemplateTiers, and allTariffTemplates are already declared in billing-calculator-php-ui-only.js
// Do NOT redeclare them here to avoid "Identifier has already been declared" errors
...
function populateTemplateDropdown(dropdownId, billingType) {
    console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 1 - Called with dropdownId="${dropdownId}", billingType="${billingType}"`);
    
    const dropdown = document.getElementById(dropdownId);
    console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 2 - Dropdown element found:`, !!dropdown);
    
    if (!dropdown) {
        console.error(`[TEMPLATE DEBUG] populateTemplateDropdown: ERROR - Dropdown "${dropdownId}" not found!`);
        return;
    }
    
    console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 3 - Current options count:`, dropdown.options.length);
    ...
    console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 4 - Options cleared, remaining:`, dropdown.options.length);
    console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 5 - allTariffTemplates:`, allTariffTemplates);
    console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 6 - allTariffTemplates length:`, allTariffTemplates?.length);
    console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 7 - Checking filterTemplatesByBillingType exists:`, typeof filterTemplatesByBillingType);
    
    // Filter templates by billing type
    const filteredTemplates = filterTemplatesByBillingType(allTariffTemplates, billingType);
    
    console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 8 - Filtered templates:`, filteredTemplates);
    console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 9 - Filtered templates count:`, filteredTemplates?.length);
    
    if (filteredTemplates.length === 0) {
        console.warn(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 10 - No templates found, adding placeholder`);
        const option = document.createElement('option');
        option.value = '';
        option.textContent = '-- No templates available for this billing type --';
        option.disabled = true;
        dropdown.appendChild(option);
        console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 11 - Placeholder added`);
    } else {
        console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 10 - Adding ${filteredTemplates.length} templates`);
        filteredTemplates.forEach((template, index) => {
            console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 10.${index + 1} - Adding template:`, template);
            const option = document.createElement('option');
            option.value = template.id;
            option.textContent = template.name + (template.region_name ? ` (${template.region_name})` : '');
            dropdown.appendChild(option);
        });
        console.log(`[TEMPLATE DEBUG] populateTemplateDropdown: Step 11 - All templates added, final options count:`, dropdown.options.length);
    }
}

// Load tariff templates and populate dropdowns
window.loadTariffTemplates = async function() {
    console.log('[TEMPLATE DEBUG] Step 1: loadTariffTemplates called');
    console.log('[TEMPLATE DEBUG] Step 2: Checking LaravelAPI exists:', typeof LaravelAPI);
    console.log('[TEMPLATE DEBUG] Step 3: Checking getTariffTemplates exists:', typeof LaravelAPI?.getTariffTemplates);
    
    try {
        console.log('[TEMPLATE DEBUG] Step 4: Calling LaravelAPI.getTariffTemplates()...');
        const templates = await LaravelAPI.getTariffTemplates();
        console.log('[TEMPLATE DEBUG] Step 5: Templates received:', templates);
        console.log('[TEMPLATE DEBUG] Step 6: Templates count:', templates?.length);
        console.log('[TEMPLATE DEBUG] Step 7: Templates type:', Array.isArray(templates));
        
        allTariffTemplates = templates;
        console.log('[TEMPLATE DEBUG] Step 8: allTariffTemplates set:', allTariffTemplates);
        console.log('[TEMPLATE DEBUG] Step 9: Checking updateTemplateDropdowns exists:', typeof updateTemplateDropdowns);
        
        // Populate dropdowns based on current billing mode
        if (typeof updateTemplateDropdowns === 'function') {
            console.log('[TEMPLATE DEBUG] Step 10: Calling updateTemplateDropdowns()...');
            updateTemplateDropdowns();
            console.log('[TEMPLATE DEBUG] Step 11: updateTemplateDropdowns completed');
        } else {
            console.error('[TEMPLATE DEBUG] ERROR: updateTemplateDropdowns is not a function!');
        }
        
        console.log('[TEMPLATE DEBUG] Step 12: Returning templates');
        return allTariffTemplates;
    } catch (error) {
        console.error('[TEMPLATE DEBUG] ERROR in loadTariffTemplates:', error);
        console.error('[TEMPLATE DEBUG] Error stack:', error.stack);
        throw error;
    }
};
...
function updateTemplateDropdowns() {
    console.log('[TEMPLATE DEBUG] updateTemplateDropdowns: Step 1 - Function called');
    console.log('[TEMPLATE DEBUG] updateTemplateDropdowns: Step 2 - allTariffTemplates:', allTariffTemplates);
    console.log('[TEMPLATE DEBUG] updateTemplateDropdowns: Step 3 - allTariffTemplates length:', allTariffTemplates?.length);
    ...
    console.log('[TEMPLATE DEBUG] updateTemplateDropdowns: Step 4 - periodContainer found:', !!periodContainer);
    console.log('[TEMPLATE DEBUG] updateTemplateDropdowns: Step 5 - isPeriodMode:', isPeriodMode);
    console.log('[TEMPLATE DEBUG] updateTemplateDropdowns: Step 6 - Checking populateTemplateDropdown exists:', typeof populateTemplateDropdown);
    
    if (isPeriodMode) {
        // Period to Period mode - show MONTHLY templates
        console.log('[TEMPLATE DEBUG] updateTemplateDropdowns: Step 7 - Populating MONTHLY templates');
        ...
        console.log('[TEMPLATE DEBUG] updateTemplateDropdowns: Step 7 - Populating DATE_TO_DATE templates');
    ...
    console.log('[TEMPLATE DEBUG] updateTemplateDropdowns: Step 8 - Completed');
}
...
window.loadTariffTemplate = async function(templateId, mode) {
    ...
    try {
        ...
        // Debug logging
        console.log('[TEMPLATE DEBUG] Template loaded:', {
            template_id: templateId,
            template_name: template.name || template.template_name,
            tiers_count: currentTemplateTiers.length,
            tiers: currentTemplateTiers,
            tiers_structure: currentTemplateTiers.length > 0 ? Object.keys(currentTemplateTiers[0]) : []
        });
...
function updateCalculateButtonState() {
    ...
    // NOTE: Sector mode input controls have been removed - sectors are derived data only
    const periodCalculateBtn = document.getElementById('calculate_btn');
    const sectorCalculateBtn = document.getElementById('sector_calculate_btn'); // May be null - sectors are now read-only
    const periodErrorDiv = document.getElementById('calculate_error');
    const sectorErrorDiv = document.getElementById('sector_calculate_error'); // May be null - sectors are now read-only
    ...
    // NOTE: Sector mode input controls have been removed - sectors are derived data only
    // Sectors are automatically calculated from period readings and cannot be edited directly
    // This code is kept for backward compatibility but the button no longer exists
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Comparison test button not available in PHP-only calculator
    
    // Load tariff templates on page load
    console.log('[TEMPLATE DEBUG] DOMContentLoaded: Step 1 - Checking loadTariffTemplates');
    console.log('[TEMPLATE DEBUG] DOMContentLoaded: Step 2 - loadTariffTemplates type:', typeof window.loadTariffTemplates);
    console.log('[TEMPLATE DEBUG] DOMContentLoaded: Step 3 - LaravelAPI type:', typeof LaravelAPI);
    
    if (typeof window.loadTariffTemplates === 'function') {
        console.log('[TEMPLATE DEBUG] DOMContentLoaded: Step 4 - Calling loadTariffTemplates()...');
        window.loadTariffTemplates().then((templates) => {
            console.log('[TEMPLATE DEBUG] DOMContentLoaded: Step 5 - Templates loaded successfully:', templates);
            console.log('[TEMPLATE DEBUG] DOMContentLoaded: Step 6 - Templates count:', templates?.length);
        }).catch((error) => {
            console.error('[TEMPLATE DEBUG] DOMContentLoaded: ERROR loading templates:', error);
            console.error('[TEMPLATE DEBUG] DOMContentLoaded: Error stack:', error.stack);
        });
    } else {
        console.error('[TEMPLATE DEBUG] DOMContentLoaded: ERROR - loadTariffTemplates is not a function!');
    }
...
// PHP-only calculator: All calculations done server-side
// No JavaScript calculator logic - only UI management
...
function updateSaveBillsButtonState() {
    ...
    if (!periodContainer || !periodBtn) return;
    
    // Check if conditions are met
    const hasAccount = typeof currentAccountData !== 'undefined' && currentAccountData !== null && currentAccountData.account;
    const hasPeriods = typeof window.getPeriods === 'function' && window.getPeriods().length > 0;
    const hasCalculatedPeriods = hasPeriods && window.getPeriods().some(p => p.usage !== null && p.usage !== undefined);
    
    if (hasAccount && hasCalculatedPeriods) {
...
// Wrap calculate to update save button state
const originalCalculate = window.calculate;
if (originalCalculate && typeof originalCalculate === 'function') {
...
window.loadPersistedBillingState = function(bills, lastFinalizedPeriod, meters) {
    ...
    const historicalPeriods = sortedBills.map((bill, index) => {
        ...
        // Calculate opening and provisional closing readings
        ...
        if (periodReadings.length > 0) {
            ...
            // If no readings in this period, use effective closing reading from previous period
        ...
        return {
            ...
            provisional_closing: closingReading,
 ...
 * 4. Allow the calculator to compute sectors, derive provisional_closing reading, determine status
 * 5. WAIT for Period 1 to fully resolve
 * 6. Pass Period 1 effective closing reading (provisional_closing or calculated_closing if reconciled) as opening for Period 2
...
window.populateCalculatorSequentially = async function(meters, accountInfo, tariffTemplate) {
    ...
    if (isDateToDate) {
            ...
            try {
                ...
                if (typeof BillingEngineLogic.calculate === 'function') {
                    try {
                        ...
                        if (calculatedPeriod) {
                            // DIRECTIVE COMPLIANCE: Use effective closing (calculated_closing if reconciled, otherwise provisional_closing)
                            const isReconciled = calculatedPeriod.is_reconciled === true;
                            const effectiveClosing = (isReconciled && calculatedPeriod.calculated_closing !== null && calculatedPeriod.calculated_closing !== undefined)
                                ? calculatedPeriod.calculated_closing
                                : (calculatedPeriod.provisional_closing ?? null);
                            console.log(`populateCalculatorSequentially: [PERIOD ${periodNumber}] Period status after calculation:`, {
                                ...
                                provisional_closing: calculatedPeriod.provisional_closing,
                                calculated_closing: calculatedPeriod.calculated_closing,
                                effective_closing: effectiveClosing,
        ...
        for (let pIdx = 1; pIdx < finalPeriods.length; pIdx++) {
            ...
            const prevIsReconciled = prevPeriod.is_reconciled === true;
            const prevEffectiveClosing = (prevIsReconciled && prevPeriod.calculated_closing !== null && prevPeriod.calculated_closing !== undefined)
                ? prevPeriod.calculated_closing
                : (prevPeriod.provisional_closing ?? null);
            if (prevEffectiveClosing !== null && prevEffectiveClosing !== undefined) {
                if (currentPeriod.opening !== prevEffectiveClosing) {
                    console.warn(`populateCalculatorSequentially: Period ${pIdx + 1} opening (${currentPeriod.opening}) does not match period ${pIdx} effective closing (${prevEffectiveClosing})`);
                }
            }
        }
