<template>
  <AdminLayout>
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">Tariff Templates</h1>
      <Link :href="route('tariff-template-create')" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
        <i class="fas fa-plus fa-sm text-white-50"></i> Add Template
      </Link>
    </div>
    <div class="card shadow mb-4">
      <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Tariff Templates List</h6>
      </div>
      <div class="card-body">
          ...
          <table class="table table-bordered" width="100%">
            ...
            <tbody>
              <tr v-for="template in templates" :key="template.id">
                ...
                <td>{{ template.is_water ? 'Water' : template.is_electricity ? 'Electricity' : 'Mixed' }}</td>
                <td>{{ template.effective_from }}</td>
                <td>{{ template.is_active ? 'Active' : 'Inactive' }}</td>
                <td>
                  <Link :href="`/admin/tariff_template/edit/${template.id}`" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></Link>
                </td>
              </tr>
              <tr v-if="templates.length === 0">
                <td colspan="6" class="text-center">No tariff templates found</td>
              </tr>
            </tbody>
          </table>
        </div>
