          ...
          <table class="table table-bordered" width="100%">
            ...
            <tbody>
              <tr v-for="page in pages" :key="page.id">
                ...
                <td>{{ page.order }}</td>
                <td>
                  <a :href="`/admin/pages/edit/${page.id}`" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></a>
                  <button @click="deletePage(page.id)" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></button>
                </td>
              </tr>
...
const deletePage = async (id) => {
  if (confirm('Are you sure?')) {
    await fetch(`/admin/pages/delete/${id}`, { method: 'GET', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content } })
