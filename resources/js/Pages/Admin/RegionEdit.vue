<template>
  <AdminLayout>
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">Edit Region / Municipality</h1>
        ...
        <form @submit.prevent="submit">
          ...
          <div class="card mb-4">
            <div class="card-header d-flex align-items-center justify-content-between">
              <h6 class="m-0 font-weight-bold text-primary">
                Billing Zones
                <span v-if="form.zones.length > 0" class="badge badge-primary ml-2">{{ form.zones.length }}</span>
              </h6>
            ...
            <div class="card-body">
              <p class="text-muted small" v-if="form.zones.length === 0">
                No zones configured. The default emails above will be used for all accounts in this municipality.
              </p>
          ...
          <div class="d-flex gap-2">
            <button type="submit" class="btn btn-warning" :disabled="saving">
              ...
              <span v-else><i class="fas fa-save fa-sm mr-1"></i> Update Region</span>
            </button>
...
const form = reactive({
  ...
  municipality:      props.region.municipality || '',
  water_email:       props.region.water_email || '',
  electricity_email: props.region.electricity_email || '',
  zones:             (props.region.zones || []).map(z => ({
    id:                z.id,
...
async function submit() {
  ...
  errors.value = {}
  try {
    await window.axios.post(route('edit-region'), {
      id:                props.region.id,
