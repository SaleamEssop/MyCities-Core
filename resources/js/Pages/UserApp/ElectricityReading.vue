<template>
  <UserAppLayout title="ENTER READINGS" :showBack="true">
    ...
    <!-- METER DISPLAY (6 integer digits, no dash) -->
    <MeterDisplay v-if="addingNew" :digits="digits" type="electricity" />

    <!-- KEYPAD — no decimal key -->
    <NumericKeypad
      v-if="addingNew"
      :showDecimal="false"
      @digit="onDigit"
    ...
    <!-- FEEDBACK -->
    <div v-if="feedbackMsg" class="reading-feedback"
         :class="feedbackOk ? 'reading-feedback--ok' : 'reading-feedback--err'">
      {{ feedbackMsg }}
    </div>
...
// 6-element digits array — right-to-left input, all integers
...
// Stored value: integer kWh, e.g. digits [0,1,2,3,3,2] → 12332 ... wait
// digits [1,2,3,3,2,7] → 123327
...
.reading-type-icon--elec { color: var(--ua-electricity, #FFA000); }
